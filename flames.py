...
def handle_feedback_photo(message):
    ...
    caption = (
        ...
        f"{HTML_EMOJI.PROFILE} <b>Username:</b> @{username}\n"
        f"{HTML_EMOJI.PROFILE} <b>ID:</b> <code>{user_id}</code>\n"
        f"{HTML_EMOJI.LINK} <b>Chat:</b> <code>{message.chat.id}</code>\n"
        f"{HTML_EMOJI.TARGET} <b>Target:</b> <code>{fb['target']}:{fb['port']}</code>\n"
        f"{HTML_EMOJI.TIMER} <b>Duration:</b> <code>{fb['duration']}s</code>\n"
        f"{HTML_EMOJI.TIMER} <b>Time:</b> <code>{datetime.now().strftime('%d-%m-%Y %H:%M:%S')}</code>"
    )
    ...
    # ✅ Confirm to user with keyboard
    safe_send_message(
        message.chat.id,
        f"{HTML_EMOJI.CHECKMARK} <b>Feedback Received!</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"{HTML_EMOJI.PARTY} Thank you for your feedback!\n"
        f"{HTML_EMOJI.FIRE} You can now start a new attack using /attack command.",
        reply_to=message,
        parse_mode="HTML"
    )
...
def id_command(message):
    if check_banned(message): return
    user_id = message.from_user.id
    safe_send_message(message.chat.id, f"{HTML_EMOJI.PROFILE} <b>Your ID:</b>: <code>{user_id}</code>", reply_to=message)
